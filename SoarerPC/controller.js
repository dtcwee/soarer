...
/*Initialise bindings*/
try {
var ctlBindings = JSON.parse(getCookie("ctlBindings"));
}
catch(error) {
	reset(defaultKeyboardBindings);
}
...
function addgamepad(gamepad) {
  ...
  document.getElementById("controllerConnected").style.display = "inline";
...
function removegamepad(gamepad) {
  ...
  delete controllers[gamepad.index];
	document.getElementById("controllerDisconnected").style.display = "inline";
...
setCookie("ctlBindings", JSON.stringify(ctlBindings), 30);
}
...
function toggleControllersDisplay() {
  if (document.getElementById('toggleDebug').checked && Object.keys(controllers).length > 0) {
